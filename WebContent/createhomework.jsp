	...
	<form action="CreateHomework" method="post">
		<%
			out.println("<input type='hidden' name='"
					+ Course.COURSE_ID_PARAM_NAME + "' value='" + courseID
					+ "'>");
		%>
		<div>
			Number: <input type="text" name="number" maxlength="2" size="2"
				required>
		</div>
		<div>
		...
		<div>
			<fieldset>
				...
				Month: <input type="text" maxlength="2" size="2" name="month"
					required> Year: <input type="text" maxlength="4" size="4"
					name="year" required> Time: <select name="hours" id="hours">
				</select> <select name="minutes" id="minutes">
				</select>
			</fieldset>
		</div>
		<div>
			<input type="checkbox" name="late_days" value="1" checked>
			Allow late days <br> <input type="checkbox" name="active"
				value="1" checked> Is active
		</div>
		<br>
